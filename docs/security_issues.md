...
| ID | Závažnost | Název | Stav | Dotčené soubory |
|---|---|---|---|---|
...
| SEC-8 | 🟡 Nezávažné | CSRF bez explicitního tokenu (zmírněno `SameSite=Lax`) | ⏸️ Odloženo | `src/lib/auth.ts`, admin routy |
| SEC-9 | 🟡 Nezávažné | Prompt injection do shrnutí poptávky zobrazovaného v adminu | ✅ Opraveno | `src/app/api/leads/route.ts`, `src/app/admin/(authenticated)/leads/client.tsx` |
| SEC-10 | 🟡 Nezávažné | Chybějící bezpečnostní HTTP hlavičky (CSP, X-Frame-Options, HSTS) | ✅ Opraveno | `next.config.ts` |
...
### Stav opravy
✅ **Opraveno** (balíček B, commit `6ae3487`). `clientIp` bere `x-real-ip` → pravou hodnotu XFF →
`unknown`; levá spoofovatelná hodnota se nepoužívá. `hits.clear()` nahrazen vystěhováním čtvrtiny
...
### Stav opravy
✅ **Opraveno** (balíček A, commit `5e9111e`). Nový helper `src/lib/require-admin.ts` (`requireAdmin()`
ověří session cookie přes `verifySession`) je volán na prvním řádku všech 8 admin handlerů. Proxy
zůstává první vrstvou. Ověřeno: s dočasně vyřazenou routou z matcheru proxy vrátil 401 sám handler;
veřejné routy nedotčeny.
...
### Stav opravy
✅ **Opraveno** (balíček C, commit `da270d8`). Surová `error.message` nahrazena generickou hláškou +
`console.error` napříč routami (feedback, documents, reprocess, leads/[id], settings, retrieval-test).
`retrieval-test` validuje `query` jako neprázdný string ≤ 4 000 znaků. Zjištění: `parseSettingsInput`
je tolerantní (clampuje, nevyhazuje), takže catch v settings řeší jen DB chybu genericky. Ověřeno
vynucenou DB chybou (500 generická, detail jen v logu) i validačními vstupy (400).
...
### Stav opravy
✅ **Opraveno** (balíček B, commit `6ae3487`). Mapa `failedAttempts` má strop `MAX_KEYS = 5000` a
vystěhování stejným vzorem jako sdílený limiter (vypršelá okna → pod limitem → nouzově, nikdy
`clear()`). Sémantika zůstala vlastní (počítají se jen selhání, úspěch nuluje per-IP záznam).
...
### Stav opravy
✅ **Opraveno** (balíček D, commit `5ec58ee`). `isAllowedFile` nahrazen funkcí `allowedExtension`, která
vždy vyžaduje příponu z whitelistu `pdf|txt|md`; MIME je jen druhotný signál (cizí konkrétní MIME jako
`application/x-msdownload` odmítne i při správné příponě). Cesta ve Storage se sestavuje výhradně
z whitelistované přípony. Deklarované PDF se ověřuje magickými bajty `%PDF`. Ověřeno integračním testem
(evil.exe/octet, evil.pdf/x, fake.pdf, x-msdownload → 400; platné → 201, cesta `file.txt`).
...
### Stav opravy
⏸️ **Odloženo** (balíček G — produkční dluh). Vyžaduje návrhové rozhodnutí (serverová historie vs. jen
uživatelské dotazy od klienta). Zbytkové riziko je nízké — žádné nástroje ani exfiltrační kanál a
systémový prompt instruuje ignorovat pokyny z dotazu (`prompts.ts`). Ponecháno současné zmírnění.
...
### Stav opravy
⏸️ **Odloženo** (balíček G — produkční dluh). `SameSite=Lax` cross-site zápisy blokuje, reálné riziko
je nízké; double-submit CSRF token je belt-and-suspenders pro produkci. Ověřeno, že žádná stavová
operace není dostupná přes GET (oba admin GET handlery jsou čistě čtecí).
...
### Stav opravy
✅ **Opraveno** (balíček F, commit `05ecea9`). Přepis se v promptu izoluje do bloku `<transcript>`;
system prompt označuje jeho obsah za nedůvěryhodný vstup (pokyny = data, ne instrukce).
`sanitizeForTranscript` neutralizuje ostré závorky, takže klient nemůže podvrhnout uzavření bloku.
Admin UI zobrazuje muted popisek „Automatické shrnutí z textu klienta — neověřené." Ověřeno
adversariálními vstupy (VIP priorita, tag-break + „HACKED") — shrnutí zůstala věcná.
...
### Stav opravy
✅ **Opraveno** (balíček E, commit `f0443af`). Do `next.config.ts` přidán `async headers()` s plošným
pravidlem: `X-Frame-Options: DENY` + `Content-Security-Policy: frame-ancestors 'none'`,
`X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`. Plná CSP
odložena (inline skripty Next.js). Ověřeno na `/` i `/admin/login`.
