...
|---|---|---|
| Full-stack framework | **Next.js 16 (App Router) + React 19 + TypeScript** | Frontend i backend (API routes) v jednom projektu; nejlépe „vibecodovatelný“ stack — AI asistenti ho znají nejlépe. |
| UI | **Tailwind CSS v4 + shadcn/ui** | Rychlé, hezké výchozí komponenty (chat, tabulky, dialogy, upload). Tailwind v4 = konfigurace přes `@theme` v `globals.css`. |
