...
- **Minimalizace (etapa E):** `record_content` je vypnutý, takže do Langfuse neodchází obsah dotazů ani odpovědí. `feedback.query` se ukládá **jen u hodnocení „down"** — u kladného hodnocení text dotazu nenese nic, co by už neneslo `session_id` a pořadí zprávy, zatímco u záporného je jedinou stopou toho, na čem bot selhal.
...
- **Transparence (etapa D):** veřejná stránka `/privacy` stojí mimo proxy matcher — subjekt se k ní musí dostat dřív, než cokoli odešle. Doby uchování **čte z `app_settings`**, takže změna lhůty v administraci se do zásad promítne sama; opsané číslo by z nich po první změně udělalo nepravdivý dokument. Ze stejného důvodu text souhlasu u poptávky lhůtu neopakuje a odkazuje na zásady. Cookie lišta se vědomě nezavádí: aplikace nepoužívá cookies ani analytiku a `kecalo_session_id` v `localStorage` je nezbytný pro funkci (deduplikace hlasů, návaznost poptávky). Protože zásady lhůty veřejně slibují, `/admin/privacy` při vypnutém úklidu upozorní, že se nedodržují.
