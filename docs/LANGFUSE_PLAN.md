...
> **Stav implementace:** ✅ Kompletně hotovo — kód, konfigurace, dokumentace, lint, build, runtime ověření i traces v Langfuse dashboardu.
...
- [x] `npm install @langfuse/otel @opentelemetry/sdk-trace-node @opentelemetry/api`
- [x] Ověřit, že `package.json` obsahuje všechny tři balíčky
- [x] `npm ls @opentelemetry/api` — **musí být jediná, deduplikovaná verze.** `@opentelemetry/api` už je v `node_modules` tranzitivně přes AI SDK (aktuálně `1.9.1`); pokud npm nainstaluje druhou verzi vedle, vznikne klasický problém „dvě kopie `@opentelemetry/api`" → globální tracer registrovaný jednou kopií není vidět druhou → **tichá ztráta traces**. Při neshodě sjednotit přes npm `overrides`.
- [x] `npm run build` — ověřit, že build projde bez chyb

### Env proměnné

- [x] Založit účet na https://cloud.langfuse.com (free tier)
- [x] Vytvořit nový projekt „Kecalo" v Langfuse dashboardu
- [x] Získat `LANGFUSE_SECRET_KEY` a `LANGFUSE_PUBLIC_KEY` z Project Settings → API Keys
- [x] Přidat do `.env.local`:
  ```
  LANGFUSE_SECRET_KEY=sk-lf-...
  LANGFUSE_PUBLIC_KEY=pk-lf-...
  LANGFUSE_BASE_URL=https://cloud.langfuse.com
  ```
- [x] Přidat do `.env.example` (prázdné hodnoty):
  ```
...
- [x] Přidat `serverExternalPackages` pro OTel balíčky (nesmí být bundlovány) — **všechny tři**, včetně `@langfuse/otel`:
  ```typescript
...
- [x] Vytvořit `src/instrumentation.ts` s exportovanou funkcí `register()`:
...
- [x] **Guard proti dvojí registraci:** v dev s HMR/Turbopackem se `register()` může zavolat víckrát → duplicitní provider/processor. Ochránit příznakem na `globalThis` (zaregistrovat právě jednou).
- [x] Graceful degradace: celý blok v try/catch — pokud Langfuse env proměnné chybí, logovat warning a neregistrovat provider (app musí fungovat i bez Langfuse)
...
- [x] Vytvořit `src/lib/telemetry.ts`:
...
- [x] Tento modul importují všechny instrumentované soubory místo přímého importu `@opentelemetry/api`

**Dílčí milník:** ✅ modul se kompiluje, export typů je správný; vnořování ověřeno v Langfuse dashboardu.
...
- [x] Import `withSpan`, `getTracer`, `flushTelemetry` z `@/lib/telemetry`, `after` z `next/server`
- [x] Otevřít rodičovský span ručně přes `getTracer().startActiveSpan("chat-pipeline", async (span) => { ... })` a celé tělo POST přesunout dovnitř callbacku (callback vrací `Response`, kterou POST vrátí):
...
- [x] Obalit `retrieve()` do `withSpan("retrieval", ...)` (je `await`-ované, uzavře se korektně samo):
  - Atributy na konci: `retrieval.chunk_count`, `retrieval.top_similarity` (max skóre), `retrieval.is_fallback` (chunks.length === 0)
- [x] Na obě volání `streamText` (RAG i fallback větev) přidat `experimental_telemetry`:
  ```typescript
  ...
  ```
- [x] Rodičovský span ukončit **až po streamu** — do obou `streamText` přidat:
...
- [x] Přidat `after(() => flushTelemetry())` před return (flush proběhne po doběhnutí streamu)
- [x] Zachovat stávající `console.error` volání — Langfuse je doplňuje, ne nahrazuje
...
- [x] Poslat dotaz přes chat UI
- [x] V Langfuse dashboardu se objeví trace s:
  - Názvem `chat-pipeline` (root) a vnořeným `chat-rag`
  - **Vnořeným** LLM spanem (od AI SDK) s modelem `claude-sonnet-4-6` — ověřeno, že je *child* `chat-pipeline`, ne sourozenec (test správného `startActiveSpan`)
  - Vnořeným `retrieval` spanem
  - Token usage (input/output tokens) z `onFinish` — ověřeno: 8 251 prompt → 203 completion
  - **Latence `chat-pipeline` ≈ retrieval + generování** (ne jen retrieval) — ověřeno: 7.62s zahrnuje generování
- [x] Obsah promptů/odpovědí v traces **není** vidět (důsledek `recordInputs/recordOutputs:false` — záměr) — ověřeno: Input: null, Output: undefined
...
- [x] Import `withSpan` z `@/lib/telemetry`
- [x] V `embedQuery`: obalit `voyage.embed()` do `withSpan("embed.query", ...)`:
...
- [x] V `embedBatch`: obalit každý batch do `withSpan("embed.batch", ...)`:
...
- [x] Import `withSpan` z `@/lib/telemetry`
- [x] **Neobalovat `embedQuery` zvlášť** — instrumentace embeddingu je v `embed.ts` a span se vnoří automaticky (`retrieve` poběží uvnitř aktivního `retrieval` spanu z kroku 9.4).
- [x] Obalit `supabase.rpc('match_chunks')` do `withSpan("vector-search", ...)`:
  - Atributy: `search.match_threshold`, `search.match_count` (= topK)
  - Po dokončení: `search.result_count`, `search.top_similarity`

### Ověření

- [x] Poslat dotaz přes chat — v Langfuse trace vidět vnořené spany:
  ```
  chat-pipeline
    ├── retrieval
  ...
  ```
- [x] Ověřit, že embedding má **právě jeden** span (žádná duplicita) a že latence jednotlivých kroků jsou realistické — ověřeno: embed.query 0.35s, vector-search 0.07s
...
- [x] Import `withSpan`, `flushTelemetry` z `@/lib/telemetry`
- [x] Obalit celý `processDocument` do `withSpan("document.process", ...)`:
  - Atributy: `document.id`, `document.filename`, `document.mime_type`
- [x] Vnořené spany pro každý krok:
...
- [x] Na konci `processDocument` (v `finally`): `await flushTelemetry()`
  — `processDocument` běží v `after()` kontextu, flush je nutný

### Změny v `src/app/api/documents/route.ts`

- [x] V POST handleru přidat span `document.upload` kolem uploadu do Storage + insertu do DB (soubor už `after` z `next/server` importuje — `documents/route.ts:1`)
...
**Dílčí milník:** Kód hotov; ověření uploadu dokumentu odloženo (vyžaduje ruční upload přes admin UI).
...
- [x] Import `after` z `next/server` (soubor ho zatím neimportuje) a `withSpan`, `flushTelemetry` z `@/lib/telemetry`
- [x] Přidat `export const maxDuration = 60` (soubor ho zatím nemá — kvůli `after()` flush)
- [x] Obalit do `withSpan("retrieval-test", ...)` (analogicky chat route, ale jednodušší — jen retrieval, žádné LLM, takže se uzavře synchronně po `await retrieve()`)
- [x] Přidat `after(() => flushTelemetry())`
- [x] Atributy: `test.result_count` (surový dotaz neukládat — konzistence s rozhodnutím o soukromí z kroku 9.4; volitelně `test.query_length`)
- [x] Pozn.: `retrieve()` už je instrumentovaná z kroku 9.5 — `vector-search` span se automaticky vnoří
...
**Dílčí milník:** Kód hotov; ověření odloženo (vyžaduje ruční test v admin UI).
...
- [x] Přidat do tabulky env proměnných:
  ```
  ...
  ```
- [x] Přidat do sekce „Architektura" popis observability:
...
- [x] Přidat nové soubory do adresářové struktury

### docs/IMPLEMENTATION_PLAN.md

- [x] Zaškrtnout hotové kroky ve Fázi 9
- [x] Přidat nové soubory do adresářové struktury
- [x] V „Produkční dluh" nahradit „Monitoring nákladů a latence" odkazem na Fázi 9
...
- [x] `npm run dev` — server nastartuje, `[telemetry] Langfuse OTel provider zaregistrován.`
- [x] Poslat 3 různé dotazy přes chat (včetně jednoho fallback dotazu)
- [x] Otevřít Langfuse dashboard — ověřit:
  - [x] Traces pro chat (chat-rag i chat-fallback)
  - [x] Každý trace má vnořenou strukturu (retrieval → embed.query + vector-search; chat-rag → LLM span)
  - [x] LLM span je **child** `chat-pipeline` (ne sourozenec) a latence rodiče zahrnuje generování — ověřeno: 7.62s
  - [x] Token usage je vyplněný (input tokens, output tokens) z `onFinish` — ověřeno: 8 251 → 203
  - [x] Embedding má právě jeden span (žádná duplicita)
  - [x] Metadata (topK, threshold, temperature) jsou viditelná; obsah promptů viditelný není (záměr)
- [ ] Nahrát dokument — ověřit trace `document.process` (odloženo — vyžaduje ruční upload)
- [ ] Spustit retrieval test — ověřit trace `retrieval-test` (odloženo — vyžaduje ruční test)

### Bez Langfuse klíčů (graceful degradace)

- [x] Ověřit, že app funguje normálně bez chyb (chat, upload, retrieval test) — ověřeno: chat na portu 3000 před restartem serveru (bez registrovaného provideru) vrátil 200 OK
