...
**GDPR (mimo číslované fáze):** uvedení zpracování osobních údajů do souladu dle `docs/plans/gdpr_plan.md`. **Hotovo — etapy A–G, E2E ověřené** (4. 9. 2026): migrace `019` (retenční parametry v `app_settings` + auditní tabulka `privacy_actions`), retenční runner `src/lib/privacy/retention.ts` se dvěma vstupními brankami (denní cron `GET /api/cron/retention` chráněný `CRON_SECRET` a ruční `POST /api/privacy/retention` pro admina), nástroj práv subjektu v `/admin/privacy` (vyhledání podle kontaktu, JSON export, trvalý výmaz) a historie úkonů. Ověřeno: bez cookie 401, cron bez/se špatným secretem 401 a bez `CRON_SECRET` 503; úklid při vypnuté retenci vrací `skipped`, při zapnuté smazal přesně předdatované testovací řádky a nové nechal; výmaz na žádost odstranil poptávku i navázaný hlas a hledání pak vrací prázdno; e-mail se najde nezávisle na velikosti písmen, telefon i s předvolbou navíc. **`retention_enabled` je po migraci vypnutá** — zapnutí je vědomé rozhodnutí správce a v produkci zatím neproběhlo. Etapa D přidala **transparenci**: veřejná stránka `/privacy` (mimo proxy matcher, dostupná bez přihlášení), přepsaný text souhlasu v `LeadForm` a odkazy v patičce chatu i widgetu. Doby uchování se na `/privacy` **čtou z `app_settings`**, ne opisují — jinak by změna lhůty v adminu udělala ze zásad nepravdivý dokument; ze stejného důvodu text souhlasu lhůtu neopakuje a odkazuje na zásady. Cookie lišta se vědomě nezavádí (žádné cookies ani analytika; `kecalo_session_id` v `localStorage` je nezbytný pro funkci). Protože zásady lhůty veřejně slibují, `/admin/privacy` při vypnutém úklidu varuje, že se nedodržují. V zásadách jsou **záměrně viditelné zástupné texty `DOPLNIT`** (identifikace správce, kontaktní e-mail, regiony Supabase a Langfuse) — tichý placeholder by se do produkce protáhl snadno. Etapa E přinesla **minimalizaci toku dat ven**: `record_content` je v produkci vypnutý (ověřeno na reálných trace přes Langfuse API — `input` i `output` jsou `null`), varování u přepínače nově říká, že jde o osobní údaje a že na obsah v Langfuse **nedosáhne výmaz na žádost**, a `feedback.query` se ukládá **jen u hodnocení „down"** (u palce nahoru text nenese nic, co by už neneslo `session_id` + `message_index` — minimalizace dle čl. 5 odst. 1 písm. c; existující řádky doběhnou retencí). Etapa F doplnila provozní příručku `docs/gdpr.md` (mapa údajů, postupy pro žádosti, zaměstnanecké účty, zpracovatelé, checklist před ostrým provozem). Etapa G přidala **provozní režim** (migrace `020`): režim se ZÁMĚRNĚ neukládá jako přepínač — hranice „smí se ptát anonym" je env `PUBLIC_CHAT` vyhodnocovaná v `proxy.ts` (fail-closed: bez `true` vyžadují `/`, `/demo`, `/api/chat`, `/api/feedback` i `POST /api/leads` přihlášení), sběr kontaktů je nezávislý parametr `lead_capture_enabled` v `/admin/privacy` a samotný režim se jen **odvozuje** komponentou `DeploymentMode` (sidebar + `/admin/privacy`), která navíc hlásí nesourodé kombinace. Právní titul se ukládá **k řádku** (`leads`/`feedback` += `processing_basis`) podle tabulky, ne podle přihlášení: poptávka vždy `souhlas`, hodnocení vždy `opravneny_zajem` — titul se váže k okamžiku sběru a po změně konfigurace musí zůstat doložitelný. Texty na `/privacy` mají interní variantu (§ 316 zákoníku práce, štítkové řízení přístupu) volenou týmž odvozeným stavem.

**Pozor při nasazení:** `PUBLIC_CHAT=true` musí být ve Vercel Project env **dřív**, než se nasadí tato změna — jinak veřejný chat přestane odpovídat anonymním návštěvníkům.
...
| `PUBLIC_CHAT` | `true` = chat je veřejný (dnešní režim). Cokoli jiného = interní režim, kdy `/`, `/demo`, `/api/chat`, `/api/feedback` i `POST /api/leads` vyžadují přihlášení. Fail-closed záměrně — zapomenutá proměnná nesmí interní bázi otevřít internetu |
...
│   ├── DeploymentMode.tsx            # odvozený provozní režim + varování o nesourodé konfiguraci (etapa G)
...
└── migrations/
    ...
    └── 020_lead_capture_and_basis.sql # app_settings += lead_capture_enabled; leads/feedback += processing_basis (etapa G)
scripts/
...
Widget nepřidává žádnou API routu ani útočnou plochu — používá výhradně existující veřejné routy (`/api/chat`, `/api/feedback`, `POST /api/leads`) se stávajícími rate limity. Fáze 2 (embeddovatelný widget na cizí web přes `/widget` + `public/embed.js`) je neimplementovaná — plán `docs/plans/widget_embed_plan.md` (bublina v shadow DOM přes `public/embed.js`, panel v iframu na `/widget`, zúžení `frame-ancestors`); ochranu nákladů z ní přebírá `docs/plans/public_chat_protection_plan.md`.
